...
| `requiresAddress` | boolean | No<br/><br/>Default value is `false`. | If `true`, Excel makes the cell's `invocationContext.address` (e.g. the location of a cell) available. For more information, see [Discovering cells that invoke custom functions](custom-functions-overview.md#discovering-cells-that-invoke-custom-functions).
